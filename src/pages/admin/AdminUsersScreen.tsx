...
import { UsersService, useAuth } from '../../context';
import type {
  CreateUserPayload,
  NormalizedStatus,
  UpdateUserPayload,
...
const normalizeStatusLabel = (status: NormalizedStatus): 'Activo' | 'Inactivo' => {
  const normalized = status?.toString().toUpperCase?.() ?? 'ACTIVE';
  return normalized === 'INACTIVE' || normalized === 'INACTIVO' ? 'Inactivo' : 'Activo';
};

const toServiceStatus = (label?: string): NormalizedStatus => {
...
const AdminUsersScreen = ({ isDarkMode }: AdminUsersScreenProps) => {
  ...
  useEffect(() => {
    ...
    setIsFetching(true);
    UsersService.fetchUsers({ signal: abortController.signal })
      .then((response) => {
  ...
  const handleCreateUser = useCallback(
    (newUser: Omit<DashboardUser, 'id' | 'lastAccess'>) => {
      void (async () => {
        ...
        setError(null);
        try {
          const created = await UsersService.createUser(buildCreatePayload(newUser));
  ...
  const handleUpdateUser = useCallback(
    (id: string, updates: Partial<Omit<DashboardUser, 'id'>>) => {
      ...
      void (async () => {
        ...
        setError(null);
        try {
          const updated = await UsersService.updateUser(id, buildUpdatePayload(merged, password));
          setUsers((prev) =>
