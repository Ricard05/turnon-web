...
const DashboardQueue = ({
  ...
  const goPreviousDoctor = () => {
    setDoctorIndex((prev) => (prev - 1 + DOCTOR_OPTIONS.length) % DOCTOR_OPTIONS.length);
  };

  const goNextDoctor = () => {
    setDoctorIndex((prev) => (prev + 1) % DOCTOR_OPTIONS.length);
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {DOCTOR_OPTIONS[doctorIndex]?.name ?? 'Sin asignar'}
            </p>
